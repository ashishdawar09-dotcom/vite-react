...
import { HeroScrub } from "./components/ui/hero-scrub"; /* NEW: scroll-driven hero */
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a1628", color: "#1a1a2e", fontFamily: "'Inter','Segoe UI',system-ui,-apple-system,sans-serif" }}>
      ...
      {/* HEADER: simplified to just the brand-pill + admin row. Athlete photo, cyan glow,
          and the big hero block were removed — the photo is now the HeroScrub image,
          the hero title text becomes HeroScrub's titleTop, and the rest moves to the
          compressed info bar below. */}
      <header style={{ background: "#070F1F", color: "#fff", padding: 0, position: "relative", borderBottom: "1px solid #1a3050" }}>
        <div style={{ position: "relative", zIndex: 3, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 28px" }}>
      ...
      {/* NEW: scroll-driven cinematic hero — replaces the in-header hero block.
          Title top auto-pulls the active tournament name; title bottom is a static tagline. */}
      <HeroScrub
        image="/images/B6.jpg"
        imageAlt="Badminton tournament hero"
        titleTop={(current?.name ?? "Badminton Championship").toUpperCase()}
        titleBottom="LIVE 2026"
        accentColor="#070F1F"
      />

      {/* NEW: compressed info bar — contains every functional element from the old
          giant hero block (DOUBLES TOURNAMENT pill, date, tournament dropdown, category
          picker, pace pill, stats), packed into a single row beneath the scroll hero. */}
      <div style={{ background: "#0a1628", borderBottom: "1px solid #1a3050", padding: "14px 28px" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 12px", borderRadius: 4, background: "rgba(0,184,255,0.12)", border: "1px solid rgba(0,184,255,0.3)" }}>
          ...
          {current?.event_date && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 4, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
              ...
              <span className="font-display" style={{ fontSize: 13, fontWeight: 600, color: "#cbd5e1", letterSpacing: 1 }}>{
                // event_date is a DATE column (no time / timezone). Appending T12:00:00 (noon, local)
                // avoids the off-by-one a bare 'YYYY-MM-DD' gives in any timezone west of UTC.
          ...
          {/* Compact stats — right-aligned, smaller than the old hero version */}
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
            {([[active.length, "Active", "#00d4ff"], [teamsView.length, "Teams", "#22c55e"], [unpaired.length, "Unpaired", "#f59e0b"]] as const).map(([v, l, c]) => (
              <div key={l} style={{ display: "inline-flex", alignItems: "baseline", gap: 6, padding: "4px 10px", background: "rgba(15,30,55,0.65)", borderRadius: 4, border: "1px solid rgba(255,255,255,0.08)", borderLeft: `3px solid ${c}` }}>
                <span className="font-display" style={{ fontSize: 18, fontWeight: 700, color: c, lineHeight: 1, fontVariantNumeric: "tabular-nums" }}>{String(v).padStart(2, "0")}</span>
                <span style={{ fontSize: 9, color: "#94a3b8", textTransform: "uppercase", letterSpacing: 1.5, fontWeight: 600 }}>{l}</span>
              </div>
            ))}
          </div>
        </div>
